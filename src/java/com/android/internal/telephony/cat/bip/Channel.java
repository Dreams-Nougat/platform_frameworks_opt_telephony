...
/**
 * Abstract Class that implements Channel.
 *
 * {@hide}
 */
abstract class Channel {
    ...
    protected boolean mIsReceiveDataTRSent = false;
    ...
    public void dataAvailable(int bufferSize) {
        ...
        CatResponseMessage resMsg = new CatResponseMessage(mBipService.mCurrentSetupEventCmd);
    ...
    /**
     * Receiver thread for UDP channel.
     *
     */
    ...
    /**
     * Receiver thread for TCP channel.
     *
     */
    protected class TcpReceiverThread implements Runnable {
        DataInputStream mDataInput;
        ...
        public void run() {
            ...
            while (!mStop) {
                ...
                try {
                    Arrays.fill(localBuffer, (byte) 0);
                    recvLen = mDataInput.read(localBuffer);
                } catch (IOException e) {
            ...
            for (;; ) {
                if (mChannelStatus == BipUtils.CHANNEL_STATUS_OPEN) {
                    ...
                    if (null == mTcpServerChannel.mInput) {
                        ...
                        mDataInput = mTcpServerChannel.mInput;
                    }
                    if (null == mTcpServerChannel.mOutput) {
                        try {
...
/**
 * Class that implements TCP server channel.
 *
 * {@hide}
 */
class TcpServerChannel extends Channel {
    ...
    public int openChannel(BipCmdMessage cmdMsg, Network network) {
        ...
        if (mChannelStatus == BipUtils.CHANNEL_STATUS_UNKNOWN ||
            ...
            mReceiverThread = new Thread(new UICCServerThread(this));
        ...
        if (true == mCloseBackToTcpListen) {
            if (BipUtils.TCP_STATUS_ESTABLISHED == mChannelStatusData.mChannelStatus) {
                try {
                    ...
                    mReceiverThread.interrupt(); //free the block state of mLock.wait()
                } catch (IOException e) {
...
/**
 * Class that implements TCP Channel.
 *
 * {@hide}
 */
class TcpChannel extends Channel {
    ...
    public int openChannel(BipCmdMessage cmdMsg, Network network) {
        int ret = ErrorValue.NO_ERROR;
        Thread openChannelThread = null;
        mNetwork = network;

        if (mLinkMode == BipUtils.LINK_ESTABLISHMENT_MODE_IMMEDIATE) {
            openChannelThread = new Thread(new Runnable() {
        ...
        do {
            if (mChannelStatus == BipUtils.CHANNEL_STATUS_OPEN) {
                try {
                    ...
                    mReceiverThread = new Thread(new TcpReceiverThread(mInput));
                    mReceiverThread.start();
                } catch (IOException e) {
        ...
        CatLog.d("[BIP]", "[TCP]receiveData " + mRxBufferCount + "/" + requestCount +
            ...
            if (mRxBufferCount >= requestCount) {
                try {
                    ...
                    CatLog.e("[BIP]", "[TCP]receiveData - IndexOutOfBoundsException");
                    e.printStackTrace();
                }
            } else {
                int needCopy = requestCount;
...
/**
 * Class that implements UDP Channel.
 *
 * {@hide}
 */
    ...
    public int openChannel(BipCmdMessage cmdMsg, Network network) {
        ...
        if (mLinkMode == BipUtils.LINK_ESTABLISHMENT_MODE_IMMEDIATE) {
            try {
                ...
                mChannelStatus = BipUtils.CHANNEL_STATUS_OPEN;
                mChannelStatusData.mChannelStatus = ChannelStatus.CHANNEL_STATUS_LINK;
                mReceiverThread = new Thread(new UdpReceiverThread(mSocket));
                mReceiverThread.start();
