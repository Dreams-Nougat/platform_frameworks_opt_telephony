...
class BipRilMessageDecoder extends StateMachine {
    ...
    private static BipRilMessageDecoder[] sInstance = null;
    ...
    public static synchronized BipRilMessageDecoder getInstance(Handler caller, IccFileHandler fh,
        ...
        if (null == sInstance) {
            ...
            sInstance = new BipRilMessageDecoder[sSimCount];
            for (int i = 0; i < sSimCount; i++) {
                sInstance[i] = null;
            }
        }

        if (slotId != SubscriptionManager.INVALID_SIM_SLOT_INDEX && slotId < sSimCount) {
            if (null == sInstance[slotId]) {
                sInstance[slotId] = new BipRilMessageDecoder(caller, fh, slotId);
            }
        } else {
    ...
    /**
     * Used for the CmdParamsReady state.
     */
