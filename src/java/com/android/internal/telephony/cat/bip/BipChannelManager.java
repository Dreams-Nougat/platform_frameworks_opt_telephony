...
/**
 * Class that implements BIP Channel Manager.
 *
 * {@hide}
 */
class BipChannelManager {
    ...
    public int acquireChannelId(int protocolType) {
        byte i = 0;
        CatLog.d("[BIP]", "acquireChannelId, protocolType " + protocolType + ",occupied " +
                 mCurrentOccupiedPSCh + "," + mCurrentOccupiedUICCSerCh);
        ...
        for (i = 0; i < MAXCHANNELID; i++) {
            if (0 == (mChannelIdPool & (0x01 << i))) {
                ...
                CatLog.d("[BIP]", "acquireChannelId, mChannelIdPool " + mChannelIdPool + ":" +
                         (i + 1));
                return i + 1;
            }
        }
    ...
    public void releaseChannelId(int cId, int protocolType) {
        try {
            ...
            if (BipUtils.TRANSPORT_PROTOCOL_SERVER == protocolType &&
                ...
                CatLog.e("[BIP]", "releaseChannelId, bad parameters.cId:" + cId + ":" +
                        mChannelIdPool);
            }
            mChannelIdPool &= (byte) (~(0x01 << (byte) (cId - 1)));
            CatLog.d("[BIP]", "releaseChannelId, cId " + cId + ",protocolType " + protocolType +
                     ",occupied " + mCurrentOccupiedPSCh + "," +
                     mCurrentOccupiedUICCSerCh + ":" + mChannelIdPool);

        } catch (IndexOutOfBoundsException e) {
            CatLog.e("[BIP]", "IndexOutOfBoundsException releaseChannelId cId=" + cId + ":" +
                    mChannelIdPool);
        }
    }
        ...
        try {
            for (i = 0; i < MAXCHANNELID; i++) {
                if (null != mChannels && null != mChannels[i] &&
                        ...
                        BipUtils.TRANSPORT_PROTOCOL_TCP_REMOTE) & mChannels[i].mProtocolType)) {
                    return true;
                }
            }
