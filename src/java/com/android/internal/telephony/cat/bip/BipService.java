...
/**
 * Class that implements BIP Service.
 *
 * {@hide}
 */
public class BipService {
    private static final boolean DBG = true;
    private static BipService[] sInstance = null;
    ...
    private boolean mIsConnMgrIntentTimeout = false;
     ...
     * @param simId sim id
     */
    public BipService(Context context, Handler handler, int simId) {
        ...
        mContext = context;
        mSlotId = simId;
        CatLog.d("[BIP]", "Construct instance sim id: " + simId);
     ...
     * @param handler handler for events
     * @param simId sim id
     * @param cmdIf commands interface
     * @param fh ICC file handler
     */
    public BipService(Context context, Handler handler, int simId, CommandsInterface cmdIf,
            IccFileHandler fh) {
        CatLog.d("[BIP]", "Construct BipService " + simId);
    ...
    /**
     * Used for instantiating the BipService.
     *
     * @param context phone app context
     * @param handler evnet handler
     * @param simId to know the index of card
     * @return The BipService object in the system for the specific simId
     */
    public static BipService getInstance(Context context, Handler handler, int simId) {
        ...
        if (null == sInstance) {
            ...
            sInstance = new BipService[sSimCount];
            for (int i = 0; i < sSimCount; i++) {
                sInstance[i] = null;
            }
        }
        if (simId < PhoneConstants.SIM_ID_1 || simId > sSimCount) {
            CatLog.d("[BIP]", "getInstance invalid sim : " + simId);
            return null;
        }
        if (null == sInstance[simId]) {
            sInstance[simId] = new BipService(context, handler, simId);
        }
        return sInstance[simId];
    }
    ...
    public static BipService getInstance(Context context, Handler handler,
        ...
        if (null == sInstance[simId]) {
           sInstance[simId] = new BipService(context, handler, simId, cmdIf, fh);
        }
        return sInstance[simId];
    }
    ...
    public void dispose() {
        ...
        if (null != sInstance) {
            if (null != sInstance[mSlotId]) {
                sInstance[mSlotId] = null;
            }
        }
        // Check if all sInstance[] is null
        for (i = 0 ; i < sSimCount ; i++) {
            if (null != sInstance[i]) {
                break;
            }
        }
        // All sInstance[] has been null, set sInstance as null
        if (i == sSimCount) {
            sInstance = null;
        }
    }
        ...
        switch (cmdParams.getCommandType()) {
            case OPEN_CHANNEL:
                ...
                if (myPhone.getServiceState().getNetworkType() <=
                    ...
                    callState = callmgr.getState();
                    CatLog.d(this, "callState" + callState);
    ...
    private void disconnect() {
        ...
        if (true == mIsOpenInProgress &&
                ...
                if (true == mBipChannelManager.isChannelIdOccupied(i)) {
                    try {
                        ...
                        if (BipUtils.TRANSPORT_PROTOCOL_UDP_REMOTE == channel.mProtocolType ||
                            ...
                            // Channel status
                            //additionalInfo[firstIdx] = (byte) 0xB8;
            ...
            if (alByte.size() > 0) {
                ...
                ((CatService) mHandler).onCmdResponse(resMsg);
            } else {
    ...
    /**
     * Handle OPEN_CHANNEL command.
     *
     * @param cmdMsg BIP command message
    ...
    public void openChannel(BipCmdMessage cmdMsg, Message response) {
        ...
        if (0 == mChannelId) {
            ...
            response.obj = cmdMsg;
            mBipSrvHandler.sendMessage(response);
            CatLog.d("[BIP]", "BM-openChannel: channel id = 0. mCurrentCmd = " + mCurrentCmd);
            return;
        }
    ...
    /**
     * Create RecvDataRunnable and run the new thread.
     *
     * @param cmdMsg BIP command message
     * @param response response message
     */
    ...
    /**
     * Create SendDataThread and run the new thread.
     *
     * @param cmdMsg BIP command message
     * @param response response message
     */
    ...
    private void newRequest() {
        ...
        mNetworkCallback = new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(Network network) {
                ...
                if (false == mIsConnMgrIntentTimeout) {
    ...
    /**
     * Used to handle SEND_DATA command in a seperate thread.
     */
    protected class SendDataThread implements Runnable {
        BipCmdMessage mCmdMsg;
        Message mResponse;
        ...
        public void run() {
            ...
            byte[] buffer = mCmdMsg.mChannelData;
            int mode = mCmdMsg.mSendMode;
            Channel lChannel = null;
            int cId = mCmdMsg.mSendDataCid;

            lChannel = mBipChannelManager.getChannel(cId);
            do {
                if (null == lChannel) { // if (mChannelId != mCmdMsg.mSendDataCid)
                    CatLog.d("[BIP]", "SendDataThread Run mChannelId != mCmdMsg.mSendDataCid");
                ...
                if (lChannel.mChannelStatus == BipUtils.CHANNEL_STATUS_OPEN) {
                    ...
                    mResponse.arg2 = lChannel.getTxAvailBufferSize();
                } else {
            ...
            mResponse.arg1 = ret;
            mResponse.obj = mCmdMsg;
            CatLog.d("[BIP]", "SendDataThread Run mBipSrvHandler.sendMessage(mResponse);");
            mBipSrvHandler.sendMessage(mResponse);
        }
    }

    /**
     * Get channel status and send response message.
     *
     * @param cmdMsg BIP command message
     * @param response response message
     */
...
//FIXME: multiple network for MSIN ?
    ...
    /**
     * Used when channel opened completed.
     *
     * @param ret result code of OPEN_CHANNEL command
     * @param lChannel the opened channel
     */
    public void openChannelCompleted(int ret, Channel lChannel) {
        ...
        if (true == mIsOpenInProgress && false == mIsConnMgrIntentTimeout) {
            mIsOpenInProgress = false;
            mIsNetworkAvailableReceived = false;
    ...
    /**
     * Used to handle connectivity change event in a seperate thread.
     */
    protected class ConnectivityChangeThread implements Runnable {
        ...
        public void run() {
            ...
            NetworkInfo info =
                    (NetworkInfo) mIntent.getExtra(ConnectivityManager.EXTRA_NETWORK_INFO);
            //FIXME
            String strSubId = mIntent.getStringExtra("subId");
            int subId = 0;
            if (null == strSubId) {
    ...
    private class RecvDataRunnable implements Runnable {
        ...
        public void run() {
            ...
            lChannel = mBipChannelManager.getChannel(mCmdMsg.mReceiveDataCid);
            if (null == lChannel) {
                errCode = ErrorValue.BIP_ERROR;
            } else {
                synchronized (lChannel.mLock) {
                    lChannel.mIsReceiveDataTRSent = false;
                }
                errCode = lChannel.receiveData(mRequestDataSize, mResult);
            }

            mCmdMsg.mChannelData = mResult.buffer;
            mCmdMsg.mRemainingDataLength = mResult.remainingCount;
            mResponse.arg1 = errCode;
            mResponse.obj = mCmdMsg;
            mBipSrvHandler.sendMessage(mResponse);
            if (null != lChannel) {
                synchronized (lChannel.mLock) {
                    CatLog.d("[BIP]", "BM-receiveData: notify waiting channel.");
                    lChannel.mIsReceiveDataTRSent = true;
