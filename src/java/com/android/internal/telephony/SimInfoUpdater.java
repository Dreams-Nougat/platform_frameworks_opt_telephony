...
public class SimInfoUpdater extends Handler {
    ...
    public SimInfoUpdater() {
        ...
        mSubscriptionController = new SubscriptionController(sContext);
    ...
    public static void setDisplayNameForNewSub(String newSubName, int subId, int newNameSource) {
        SubInfoRecord subInfo = SubscriptionController.getSubInfoUsingSubId(sContext, subId);
        if (subInfo != null) {
            ...
            if (oldSubName == null ||
                (oldNameSource == SubscriptionController.DEFAULT_SOURCE && newSubName != null) ||
                (oldNameSource == SubscriptionController.SIM_SOURCE && newSubName != null && !newSubName.equals(oldSubName))) {
                SubscriptionController.setDisplayName(sContext, newSubName, subInfo.mSubId, newNameSource);
            }
        } else {
    ...
    synchronized public void updateSimInfoByIccId() {
        ...
        for (int i = 0; i < PROJECT_SIM_NUM; i++) {
            oldIccId[i] = null;
            List<SubInfoRecord> oldSubInfo = SubscriptionController.getSubInfoUsingSimId(sContext, i);
            if (oldSubInfo != null) {
                ...
                if (sInsertSimState[i] != SIM_NOT_CHANGE) {
                    ...
                    value.put(SubscriptionController.SIM_ID, SubscriptionController.SIM_NOT_INSERTED);
                    contentResolver.update(SubscriptionController.CONTENT_URI, value,
                                                SubscriptionController._ID + "=" + Long.toString(oldSubInfo.get(0).mSubId), null);
                }
            } else {
                if (sInsertSimState[i] == SIM_NOT_CHANGE) {
        ...
        for (int i = 0; i < PROJECT_SIM_NUM; i++) {
            if (sInsertSimState[i] == SIM_NOT_INSERT) {
                ...
                if (sInsertSimState[i] > 0) {
                    ...
                    SubscriptionController.addSubInfoRecord(sContext, sIccId[i] + Integer.toString(sInsertSimState[i]), i);
                    logd("SUB" + (i + 1) + " has invalid IccId");
                } else if (sInsertSimState[i] == SIM_CHANGED) {
                    SubscriptionController.addSubInfoRecord(sContext, sIccId[i], i);
                }
        ...
        List<SubInfoRecord> subInfos = SubscriptionController.getActivatedSubInfoList(sContext);
            ...
            if (hasSimRemoved) {
                ...
                for (i=0; i < PROJECT_SIM_NUM; i++) {
                    if (sInsertSimState[i] == SIM_REPOSITION) {
                        ...
                        setUpdatedData(SubscriptionController.EXTRA_VALUE_REPOSITION_SIM, nSubCount, nNewSimStatus);
                        break;
                    }
                }
                if (i == PROJECT_SIM_NUM) {
                    ...
                    setUpdatedData(SubscriptionController.EXTRA_VALUE_REMOVE_SIM, nSubCount, nNewSimStatus);
                }
            } else {
                ...
                if (i == PROJECT_SIM_NUM) {
                    ...
                    setUpdatedData(SubscriptionController.EXTRA_VALUE_NOCHANGE, nSubCount, nNewSimStatus);
                }
            }
        } else {
            logd("New SIM detected");
            setUpdatedData(SubscriptionController.EXTRA_VALUE_NEW_SIM, nSubCount, nNewSimStatus);
        }
    ...
    private static void setUpdatedData(int detectedType, int subCount, int newSimStatus) {
        ...
        if (detectedType == SubscriptionController.EXTRA_VALUE_NEW_SIM ) {
            intent.putExtra(SubscriptionController.INTENT_KEY_DETECT_STATUS, SubscriptionController.EXTRA_VALUE_NEW_SIM);
            intent.putExtra(SubscriptionController.INTENT_KEY_SIM_COUNT, subCount);
            intent.putExtra(SubscriptionController.INTENT_KEY_NEW_SIM_SLOT, newSimStatus);
        } else if (detectedType == SubscriptionController.EXTRA_VALUE_REPOSITION_SIM) {
            intent.putExtra(SubscriptionController.INTENT_KEY_DETECT_STATUS, SubscriptionController.EXTRA_VALUE_REPOSITION_SIM);
            intent.putExtra(SubscriptionController.INTENT_KEY_SIM_COUNT, subCount);
        } else if (detectedType == SubscriptionController.EXTRA_VALUE_REMOVE_SIM) {
            intent.putExtra(SubscriptionController.INTENT_KEY_DETECT_STATUS, SubscriptionController.EXTRA_VALUE_REMOVE_SIM);
            intent.putExtra(SubscriptionController.INTENT_KEY_SIM_COUNT, subCount);
        } else if (detectedType == SubscriptionController.EXTRA_VALUE_NOCHANGE) {
            intent.putExtra(SubscriptionController.INTENT_KEY_DETECT_STATUS, SubscriptionController.EXTRA_VALUE_NOCHANGE);
        }

        logd("broadcast intent ACTION_SIMINFO_UPDATED : [" + detectedType + ", " + subCount + ", " + newSimStatus+ "]");
