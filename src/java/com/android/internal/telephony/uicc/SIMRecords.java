...
public class SIMRecords extends IccRecords {
    ...
    private static final int EVENT_GET_FPLMN_SYNC_DONE = 43 + SIM_RECORD_EVENT_BASE;
    ...
    public void handleMessage(Message msg) {
        ...
        try { switch (msg.what) {
            ...
            case EVENT_GET_FPLMN_SYNC_DONE:
                ...
                log("Received Updated FPLMNs");
                msg.obj = mFplmns = parseBcdPlmnList(data, "Forbidden");
                synchronized (msg) {
                    msg.notifyAll();
                }
                log("Received Updated FPLMNs 2");
                break;
    ...
    public String[] getForbiddenPlmns() {
        Message response = obtainMessage(EVENT_GET_FPLMN_SYNC_DONE);
     ...
     */
    private String[]
    parseBcdPlmnList(byte[] data, String description) {
        final int PACKED_BCD_PLMN_LEN_BYTES = 3;
        log("Received " + description + " PLMNs, raw=" + IccUtils.bytesToHexString(data));
        if(data.length == 0 || (data.length % PACKED_BCD_PLMN_LEN_BYTES ) != 0) {
            loge("Received invalid " + description + " PLMN list");
            return null;
        }
        int numPlmns = data.length / PACKED_BCD_PLMN_LEN_BYTES;
        String[] ret = new String[numPlmns];
        for(int i = 0; i < numPlmns; i++) {
            ret[i] = IccUtils.bcdPlmnToString(data, i * PACKED_BCD_PLMN_LEN_BYTES);
        }
        if (VDBG) log(description + " PLMNs: " + Arrays.toString(ret));
        return ret;
    }
    /**
