...
public class UiccCard {
    ...
    public void update(Context c, CommandsInterface ci, IccCardStatus ics) {
        synchronized (mLock) {
            ...
            RadioState power = mCi.getRadioState();
            if (DBG) log("update: radioState=" + power);
            // No notifications while radio is off or we just powering up
            if (power == RadioState.RADIO_ON && mLastRadioState == RadioState.RADIO_ON) {
                ...
                if (DBG) log("update: power is off, ignore");
            }
            mLastRadioState = power;
        }
    }
