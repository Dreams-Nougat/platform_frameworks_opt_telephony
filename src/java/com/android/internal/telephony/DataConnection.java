...
public abstract class DataConnection extends StateMachine {
    ...
    public enum FailCause {
        ...
        // specified here
        ROUTING_AREA_UPDATE_REJECT(0xFF),
        UNKNOWN(0x10000),
        ...
        private boolean isTemporaryFail() {
            if (sTempFailCauses != null) {
                 for (FailCause f : sTempFailCauses) {
    ...
    private static ArrayList<FailCause> sTempFailCauses;
    static {
        String prop = SystemProperties.get("ro.net.data_tmpfailures");

         if (!TextUtils.isEmpty(prop)) {
             String strTempFailCausesArray[] = prop.split(",");
             if (strTempFailCausesArray.length > 0) {
                 sTempFailCauses = new ArrayList<FailCause>();
                 for (String s : strTempFailCausesArray) {
                     try {
                         FailCause failCause = FailCause.fromInt(Integer.parseInt(s.trim()));
                         if (failCause != FailCause.UNKNOWN) {
                             sTempFailCauses.add(failCause);
                         }
