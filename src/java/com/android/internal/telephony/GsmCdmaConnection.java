...
public class GsmCdmaConnection extends Connection {
    ...
    private boolean mGsmEnableHdIcon;
    ...
    public GsmCdmaConnection (GsmCdmaPhone phone, DriverCall dc, GsmCdmaCallTracker ct, int index) {
        ...
        if (mGsmEnableHdIcon) updateAudioQuality();
    }
    ...
    private void initConfig(GsmCdmaPhone phone) {
        mGsmEnableHdIcon = getBooleanCarrierConfig(phone.getContext(),
                phone.getSubId(), CarrierConfigManager.KEY_GSM_ENABLE_HD_ICON_BOOL);
    }
    ...
    update (DriverCall dc) {
        ...
        if (!equalsHandlesNulls(mCodec, dc.codec) && mGsmEnableHdIcon) {
    ...
    private void updateAudioQuality() {
        ...
        if (mCodec != null && mCodec.equals("Codec=AMR_WB") && mGsmEnableHdIcon) {
